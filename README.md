...
1. Every question-and-answer pair is stored in a local SQLite database.
2. When you type a message, it compares your text against known patterns using `difflib` similarity scoring.
3. The best match's response is returned. If a response gets picked often, it's tracked as more "popular."
4. You can manually add new Q&A pairs at any time — the bot doesn't learn on its own from ordinary conversation, but it does grow its knowledge base as you teach it.
